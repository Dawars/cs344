...
// from https://stackoverflow.com/questions/39980645/enable-code-indexing-of-cuda-in-clion
...
void your_gaussian_blur(const uchar4 * const h_inputImageRGBA,
    ...
    int gridX = (int)ceil(numCols / (float) blockSize.x);
    int gridY = (int)ceil(numRows / (float) blockSize.y);
